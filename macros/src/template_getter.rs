...
use syn::{Attribute, DeriveInput, Expr, Visibility};

use crate::utils::{
    expr_to_string, retrieve_attr_fields, retrieve_getter_attr_values, retrieve_struct_fields,
...
pub(crate) fn derive_template_getter_impl(input: DeriveInput) -> syn::Result<TokenStream2> {
    ...
    for field_attr in field_attrs.iter() {
        getter_methods.push(derive_getter_method(vis, &member, field_attr)?);
    }
